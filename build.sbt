...
name := "AuthorizationInAkkaWithJWT"
version := "0.1"
...
lazy val common = (
  baseProject("common")
    settings(libraryDependencies ++= compile(commonModuleDependencies) ++  test(Libraries.scalaTest,  Libraries.mock) ++ dbDependencies,
...
lazy val admin = (
  baseProject("admin")
    settings(libraryDependencies ++= Libraries.circe ++ test(Libraries.scalaTest, Libraries.akkaHttpTestKit,
